...
struct Palette: View {
    ...
    var body: some View {
        GeometryReader { geo in
            let currentBubbleSize = min(235, geo.size.width * 0.36)
            ZStack {
                Color.white.ignoresSafeArea()
                
                // Visual representation of selected emotions
                bubblesSection(size: currentBubbleSize)
    ...
    private func bubblesSection(size: CGFloat) -> some View {
        ZStack {
                ...
                Circle()
                    ...
                    .overlay(
                        Text(namedColor.name)
                            .font(.system(size: max(12, size * 0.13), weight: .black, design: .rounded))
                            .lineLimit(1)
                    ...
                    .offset(
                        x: CGFloat(index - (selectedNamedColors.count - 1)) * (size * spacingFactor)
                        + CGFloat(selectedNamedColors.count - 1) * (size * (spacingFactor / 2)),
