...
struct SoulAssemblyView: View {
    ...
    @State private var activeFragmentID: UUID? = nil
            ...
            ZStack {
                ...
                VStack {
                    ...
                    HStack(alignment: .center) {
                        ...
                        Text("Double-tap a fragment to remove it from screen")
                            .font(.system(size: isPad ? 18 : 14, design: .serif))
                            .italic()
                            .foregroundColor(.black.opacity(0.6))
                            .padding(.horizontal, 20)
                            .transition(.opacity)
                            .animation(.default, value: placedInstances.isEmpty)
    ...
    private var headerText: some View {
        VStack(spacing: 10) {
            ...
            Text("Choose a fragment and tap it. Once it appears on the canvas, drag it to any position you like.")
    ...
    private func fragmentNode(instance: FragmentInstance) -> some View {
        ...
        Image(uiImage: instance.image)
            ...
            // Restrict hit testing to the fragment polygon instead of its image rectangle.
            .contentShape(instance.relativePath)
            // Scale the fragment before positioning it so the lift animation stays
            // centered on the fragment rather than the canvas.
            .scaleEffect(instance.offset == .zero ? 1.0 : 1.1)
            .shadow(
            ...
            .gesture(
                DragGesture()
                    .onChanged { value in
                        guard let index = placedInstances.firstIndex(where: { $0.id == instanceID }) else { return }
                        activeFragmentID = instanceID
                        placedInstances[index].offset = value.translation
                    }
                    .onEnded { value in
                        defer { activeFragmentID = nil }
                        guard let index = placedInstances.firstIndex(where: { $0.id == instanceID }) else { return }

                        placedInstances[index].position.x += value.translation.width
                        placedInstances[index].position.y += value.translation.height
                        placedInstances[index].offset = .zero

                        // Move selected item to front of ZStack.
                        let movedItem = placedInstances.remove(at: index)
            ...
            // Raise the fragment as soon as dragging starts. The array reorder on
            // drop then preserves that frontmost order after the gesture ends.
            .zIndex(activeFragmentID == instanceID ? 1 : 0)
    }
                ...
                HStack(spacing: 15) {
                    ForEach(processedFragments) { item in
                        Button(action: {
                            ...
                            let imageSize = item.croppedImage.size
                            let scale = min(
                                unifiedSize.width / imageSize.width,
                                unifiedSize.height / imageSize.height
                            )
                            let displayedSize = CGSize(
                                width: imageSize.width * scale,
                                height: imageSize.height * scale
                            )
                            let displayedPath = item.relativeMaskPath.applying(
            ...
            for (index, frag) in fragments.enumerated() {
                ...
                if let contentBox = uncropped.findActualContentRect(), contentBox.width >= 25, contentBox.height >= 25 {
                    if let cg = uncropped.cgImage?.cropping(to: contentBox) {
                        // Convert the original mask into coordinates relative to the cropped image.
                        // `contentBox` is measured in pixels while SwiftUI paths use points.
                        let pathInUncropped = frag.maskPath.applying(
                            CGAffineTransform(translationX: -bounds.origin.x, y: -bounds.origin.y)
                        )
                        let relativeMaskPath = pathInUncropped.applying(
                            CGAffineTransform(
                                translationX: -contentBox.origin.x / imgScale,
                                y: -contentBox.origin.y / imgScale
                            )
                        )
                        results.append(ProcessedFragment(
                            fragment: frag,
                            croppedImage: UIImage(cgImage: cg, scale: imgScale, orientation: .up),
